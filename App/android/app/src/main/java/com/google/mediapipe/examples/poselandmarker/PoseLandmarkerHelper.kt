...
class PoseLandmarkerHelper(
                ...
                ?.let { frame ->
                    ...
                    poseLandmarker?.detectForVideo(mpImage, timestampMs)
                        ?.let { detectionResult ->
                            ...
                            logLandmarkPositions(listOf(detectionResult))
    ...
    private fun logLandmarkPositions(results: List<PoseLandmarkerResult>) {

        for (result in results) {
            for (poseLandmark in result.landmarks()) {


                for ((index, landmark) in poseLandmark.withIndex()) {
                    Log.d(TAG, "Landmark $index: x=${landmark.x()}, y=${landmark.y()}, z=${landmark.z()}")
                }
            }
        }
    }
